...
namespace CodecLib
{
   public class Steim1 {
	...
	public static int[] Decode(sbyte[] b, int numSamples, bool swapBytes, int bias)  {
        ...
        if (current != numSamples) {
            throw new Exception( "SteimException(Number of samples decompressed doesn't match number in header: "+current+" != "+numSamples);
        }
	...
	*/
	...
	public static SteimFrameBlock Encode(int[] samples, int frames, int bias, int offset)  {
		...
		if (frames <= 0) {
			throw new Exception(" SteimException(number of frames is not a positive value");
		}
		if (frames > 63) {
			throw new Exception("SteimException(cannot encode more than 63 frames, you asked for " + frames);
		}
		if (offset < 0) {
		    throw new Exception("SteimException(Offset cannot be negatuve: "+offset);
		}
		if (offset >= samples.Length) {
		    throw new Exception("SteimException(Offset bigger than samples array: "+offset+" >= "+samples.Length);
		}
	...
	protected static int[] ExtractSamples(sbyte[] bytes,
		...
		int[] out_1 = new int[currNum];
		//System.arraycopy(temp, 0, out_1, 0, currNum);  // trim array to number of values
		Array.Copy(temp, 0, out_1, 0, currNum);
        return out_1;
	}
