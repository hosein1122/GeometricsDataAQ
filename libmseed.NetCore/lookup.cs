...
namespace libmseedNetCore
{
    ...
    public class lookup
    {
         ...
         ***************************************************************************/
        private string ms_encodingstr(char encoding)
        {
           int encoding_number = (int)encoding;
            switch (encoding_number)
            {
                case 0:
                    return "ASCII text";
                case 1:
         ...
         * type is unknown.
         ***************************************************************************/
        private string ms_blktdesc(ushort blkttype)
        {
            switch (blkttype)
            {
                case 100:
         ...
         * type unknown.
         ***************************************************************************/
        private ushort ms_blktlen(ushort blkttype, char[] blkt, bool? swapflag=null)
        {
            ushort blktlen = 0;
            ...
            switch (blkttype)
            {
                ...
                case 2000: // Opaque Data
                    ...
                    if (blkt.Length>0)
                    {
                        ...
                        lenBytes[0] = BitConverter.GetBytes(blkt[4])[0];
                        lenBytes[1] = BitConverter.GetBytes(blkt[5])[0];
                        blktlen = (ushort)BitConverter.ToInt16(lenBytes);
                        if (swapflag != null)
                        {
                           gswap.ms_gswap2(ref blktlen);
                        }
                    }
                    break;
            } // end switch
         ...
         * code is unknown.
         ***************************************************************************/
        private string ms_errorstr(int errorcode)
        {
            switch (errorcode)
            {
                case MS_ENDOFFILE:
